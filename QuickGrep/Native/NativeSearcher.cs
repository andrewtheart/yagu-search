...
namespace QuickGrep.Native;

/// <summary>
/// P/Invoke wrapper around the <c>quickgrep_core</c> Rust cdylib. The DLL is
...
internal static partial class NativeSearcher
{
    ...
    // ── Session API (ABI v2): compile once, search many ──
    ...
    private static bool TryLoad()
    {
        ...
        if (!string.IsNullOrEmpty(dir))
        {
            NativeLibrary.SetDllImportResolver(typeof(NativeSearcher).Assembly, (name, asm, _) =>
            {
                ...
                var candidate = Path.Combine(dir, "quickgrep_core.dll");
                if (File.Exists(candidate) && NativeLibrary.TryLoad(candidate, out var h)) return h;
                return NativeLibrary.TryLoad("quickgrep_core", asm, null, out var h2) ? h2 : IntPtr.Zero;
            });
        }
        ...
        catch (EntryPointNotFoundException ex) { LogService.Instance.Warning("NativeSearcher", "quickgrep_core.dll missing entry point", ex); return false; }
    }
