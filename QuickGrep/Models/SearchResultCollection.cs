...
public sealed class SearchResultCollection
{
    ...
    {
        var filtered = _allGroups.Where(MatchesFilter);
        ...
        if (GroupByDirectory)
        {
            ...
            {
                1 => ascending ? byDirectory.ThenBy(group => group.LastModified) : byDirectory.ThenByDescending(group => group.LastModified),
                2 => ascending ? byDirectory.ThenBy(group => group.FileSize) : byDirectory.ThenByDescending(group => group.FileSize),
        ...
        else
        {
            sorted = SortModeIndex switch
            {
                1 => ascending ? filtered.OrderBy(group => group.LastModified) : filtered.OrderByDescending(group => group.LastModified),
                2 => ascending ? filtered.OrderBy(group => group.FileSize) : filtered.OrderByDescending(group => group.FileSize),
                _ => ascending ? filtered.OrderBy(group => group.MatchCount) : filtered.OrderByDescending(group => group.MatchCount),
            };
        }
