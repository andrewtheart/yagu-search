...
public partial class App : Application
{
    ...
    public static string CrashLogPath { get; } = Path.Combine(
        Path.GetDirectoryName(Environment.ProcessPath) ?? AppContext.BaseDirectory, "quickgrep-crash.log");
    ...
    private async Task ShowUnhandledExceptionDialogAsync(string source, Exception? exception)
    {
        try
        {
            ...
            {
                Text = "QuickGrep caught an unexpected error. The details were also written to the crash log.",
    ...
    private static void ShowUnhandledExceptionMessageBox(string source, Exception? exception)
    {
        try
        {
            ...
            _ = MessageBoxW(IntPtr.Zero, details, "QuickGrep unexpected error", 0x00000010 | 0x00002000);
        }
