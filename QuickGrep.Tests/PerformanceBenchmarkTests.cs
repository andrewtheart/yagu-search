...
///
/// Results are appended as JSON lines to <c>QuickGrep.Benchmarks/results/perf-baselines.jsonl</c>
...
public sealed class PerformanceBenchmarkTests : IDisposable
{
    ...
    public async Task HalfMillionFiles_Throughput()
    {
        ...
        // of QUICKGREP_PERF_FILE_COUNT. This is the primary scale stress test.
    ...
    private void AppendMetricsToBaseline(PerfMetrics metrics)
    {
        try
        {
            // Write to the QuickGrep.Benchmarks/results folder (gitignored).
            // Walk up from the test assembly bin dir to the solution root,
            // then into QuickGrep.Benchmarks/results.
            ...
            var baselineDir = Path.Combine(solutionRoot, "QuickGrep.Benchmarks", "results");
