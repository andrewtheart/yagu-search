...
use yagu_core::scan::{build_matcher, scan_bytes_with_matcher_ex, MatchRecord, ScanOptions};
use yagu_core::scan_grep::GrepSession;
use walkdir::WalkDir;
        ...
        let run_intree = |bytes: &[u8],
            ...
            *ns += t.elapsed().as_nanos();
            match res {
                ...
                Err(yagu_core::scan::ScanError::BinarySkipped) => *out_skipped = true,
                Err(_) => {}
            }
        };
