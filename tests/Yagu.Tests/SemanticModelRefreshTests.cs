...
public sealed class SemanticModelRefreshTests
{
    ...
    {
        string src = File.ReadAllText(Path.Combine(FindRepoRoot(), "src", "Yagu", "Services", "Ai", "FoundryLocalSemanticQueryTranslator.cs"));
    ...
    {
        string settings = File.ReadAllText(Path.Combine(FindRepoRoot(), "src", "Yagu", "UI", "Windows", "Settings", "SettingsWindow.xaml.cs"));
