...
| --- | --- | --- |
| [**x64** — YaguSetup-1.0.0.2337-x64.exe](https://github.com/andrewtheart/yagu-search/raw/main/installer/YaguSetup-1.0.0.2337-x64.exe) (~90 MB) | Most modern PCs: 64-bit Intel/AMD Windows. Start here if unsure. | Works. Defaults to the PaddleOCR engine; the OCR runtime and English models download once on first use. |
| [**x64 · Offline** — YaguSetup-1.0.0.2337-x64-offline.exe](https://github.com/andrewtheart/yagu-search/raw/main/installer/YaguSetup-1.0.0.2337-x64-offline.exe) (~464 MB) | Same as x64, but for machines that must run fully **offline** — air-gapped PCs, or to skip first-run downloads. Bundles everything needed offline: the voidtools **Everything** installer (installs with your consent, no download) and the Microsoft Edge **WebView2** runtime for the embedded terminal. | Works with **no download** — both engines ship inside the installer. **Tesseract is the default for this edition** because it is fully self-contained and needs no network; PaddleOCR is bundled too and can be selected in Settings. See the note below. |
| [**Arm64** — YaguSetup-1.0.0.2337-arm64.exe](https://github.com/andrewtheart/yagu-search/raw/main/installer/YaguSetup-1.0.0.2337-arm64.exe) (~89 MB) | Windows on ARM: Snapdragon-based laptops, Surface Pro X, Windows Dev Kit. | Works. Defaults to PaddleOCR; the OCR runtime and models download once on first use. |
| [**x86** — YaguSetup-1.0.0.2337-x86.exe](https://github.com/andrewtheart/yagu-search/raw/main/installer/YaguSetup-1.0.0.2337-x86.exe) (~70 MB) | 32-bit Windows. | Works. Defaults to the Tesseract engine (PaddleOCR's runtime is x64-only); language data downloads once on first use. |
...
> **Why is Tesseract the default engine for the Offline edition?** Tesseract is fully self-contained: its native engine binaries are bundled directly inside the installer, so image-text (OCR) search works on a completely air-gapped machine with zero network access. PaddleOCR normally downloads its native runtime and recognition models on first use; the Offline edition bundles those too, but Tesseract is chosen as the default so OCR is *guaranteed* to work offline out of the box. You can switch to PaddleOCR at any time in **Settings → OCR**. On CPU — where both engines run in Yagu — PaddleOCR is typically faster, which is why it remains the default on the online (non-offline) editions.
