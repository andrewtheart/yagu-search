...
- **Yagu.Benchmarks** — hybrid BenchmarkDotNet console + xUnit host for the `[Trait("Category","Slow")]` throughput benchmarks (writes baselines to `tests/Yagu.Benchmarks/results/`).
...
- **src/cloud/Yagu.TelemetryFunction** — Azure Function backing opt-in telemetry (ships offline/disabled by default).
- Vendored under `src/vendor/`: TextControlBox-WinUI (built-in editor), PaddleSharp (OCR), SharpSevenZip (archive search).

## Where things live in `src/Yagu/`
...
- [.github/instructions/testing.instructions.md](instructions/testing.instructions.md) — source-pin vs unit tests, compiling engine source into Yagu.Tests, coverage runsettings (applies under `tests/Yagu.Tests/`, `tests/Yagu.Benchmarks/`).
...
- [.github/instructions/semantic-search.instructions.md](instructions/semantic-search.instructions.md) — on-device NL→search pipeline, model selection, snapshot/persist guard (applies under `src/Yagu/Services/Ai/`).
- [.github/instructions/ocr.instructions.md](instructions/ocr.instructions.md) — out-of-process OCR worker, JSON-over-stdio protocol, PID-scoped cache (applies under `src/Yagu/Services/Ocr/`, `src/Yagu.OcrWorker/`).
...
- If the user asks to build, rebuild, validate a change, or launch after changes without explicitly saying Release, build **Debug** with the Rust profiling profile **and skip the version bump**: `dotnet build src/Yagu/Yagu.csproj -c Debug -p:RustProfile=profiling -p:SkipYaguVersionIncrement=true`.
- **Always pass `-p:SkipYaguVersionIncrement=true` on local Debug/validation/launch builds.** Without it, every build auto-increments `src/Yagu/Properties/build-version.txt` + `AppInfo.g.cs`, which then has to be reverted (wasted work). The flag suppresses that churn entirely, so there is nothing to revert. Do NOT pass it for Release/installer builds (they must bump so artifact names match the binary).
- Build **Release** only when the user explicitly asks for a Release build. For Release builds, use the normal Release profile without Rust profiling (and without the skip flag): `dotnet build src/Yagu/Yagu.csproj -c Release`.
...
- When investigating a `yagu_core.dll` native crash, WER crash dump, native stack, Rust FFI issue, or native search performance problem, build **Debug** with the Rust profiling profile so the app output contains a symbol-rich native binary and PDB: `dotnet build src/Yagu/Yagu.csproj -c Debug -p:RustProfile=profiling -p:SkipYaguVersionIncrement=true`.
...
- Do not build Release for crash/profiling validation unless the user explicitly asks for Release. If they ask for Release, use `dotnet build src/Yagu/Yagu.csproj -c Release` without Rust profiling unless they explicitly request Release with Rust profiling.
...
- Yagu builds auto-increment `src/Yagu/Properties/build-version.txt` and `src/Yagu/Properties/AppInfo.g.cs`. Pass `-p:SkipYaguVersionIncrement=true` (as above) to suppress that churn on validation builds so there is nothing to revert. Only Release/installer builds — or an explicit user-requested version bump — should omit the flag; if you do produce churn that way and the user didn't ask for a bump, revert it with `git checkout -- src/Yagu/Properties/build-version.txt src/Yagu/Properties/AppInfo.g.cs`.
...
- **Always ask before running tests:** When the user asks to run tests, ask whether they want the **iterative** version (`--filter "Category!=Slow&Category!=GPU&Category!=Headed"`, ~22 seconds, skips benchmarks and UI tests) or the **full** version (all 2028 tests, ~14 minutes including performance benchmarks). If you decude to run tests on your own, pick the iterative version.
- **Test categories:** `[Trait("Category","Slow")]` = perf/UI/large-corpus tests that take minutes; `[Trait("Category","GPU")]` = tests that need a GPU + a local Foundry model (currently `SemanticEvalGoldenTests`, which is tagged BOTH Slow and GPU); `[Trait("Category","Headed")]` = tests that launch the WinUI app and drive it via UIAutomation (`MultilineGuiRegressionTests`, and `MatchNavRegressionTests` which is also Slow). **Headed tests self-gate at runtime via `HeadedTestEnvironment.CanRun`: they auto-run on an interactive Windows desktop and auto-skip in CI / non-interactive / non-Windows** (GitHub Actions is detected and skipped) — so a normal `dotnet test` on a dev box exercises them, while CI quietly skips them (CI keeps `--filter "Category!=Slow&Category!=GPU"`, no Headed exclusion needed). The iterative run adds `&Category!=Headed` only to avoid the GUI window popping up during a fast inner loop: `--filter "Category!=Slow&Category!=GPU&Category!=Headed"`. Run only the GUI tests with `--filter "Category=Headed"` (on an interactive Windows desktop with no other Yagu instance running — single-instance would hijack the launch); GPU tests with `--filter "Category=GPU"`. (`MatchNavRegressionTests` keeps an extra `YAGU_RUN_UI_REGRESSION=1` opt-in on top of the capability gate because it is screenshot-fragile.)
