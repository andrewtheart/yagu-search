...
#if NET8_0_OR_GREATER
                var procAddr = NativeMethods.GetProcAddress(_modulePtr, "CreateObject");
                if (procAddr == IntPtr.Zero)
                {
                    throw new SharpSevenZipLibraryException("Failed to find CreateObject in 7-zip library.");
                }
                var createObject = (delegate*<ref Guid, ref Guid, out nint, int>)procAddr;
