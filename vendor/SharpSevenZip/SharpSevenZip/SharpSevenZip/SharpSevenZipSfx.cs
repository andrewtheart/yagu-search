...
public class SharpSevenZipSfx
{
    ...
    private void ValidateSettings(SfxSettings settings)
    {
        ...
        {
            // Even custom modules get basic settings validation — reject empty keys
            foreach (var command in settings.Keys)
            {
                if (string.IsNullOrWhiteSpace(command))
                {
                    throw new SharpSevenZipSfxValidationException("Settings key must not be null or empty.");
                }
            }
            return;
        }
    ...
    private static Stream GetSettingsStream(SfxSettings settings)
    {
        ...
        foreach (var command in settings.Keys)
        {
            // Escape double quotes in values to prevent SFX config injection
            var escapedValue = settings[command].Replace("\"", "\\\"");
            buf =
    ...
    {
        if (string.IsNullOrWhiteSpace(sfxFileName))
        {
            throw new ArgumentException("SFX output file name must not be null or empty.", nameof(sfxFileName));
        }
        // Ensure the output path is fully qualified to prevent path traversal
        sfxFileName = Path.GetFullPath(sfxFileName);
