...
- **Content index onboarding** — optionally index one or more folders you search often, choose which automatic build trigger(s) keep them up to date (or leave it Manual), and confirm the **update mode** those passes use. When you pick an automatic trigger, **Automatic incremental** is offered as the default (you can still override it) — otherwise the default **Manual full rebuild** would only ever create *missing* indexes and leave existing ones to go stale.
