...
| Index Size Management | How each index reclaims storage. Every incremental update appends a delta segment, and storage comes back through **coalescing** (merging a bounded run of neighbouring small segments) or **compaction** (streaming every active layer into a fresh base). The **default size-management strategy** chooses *Coalesce, then compact* (default), *Coalesce only*, *Compact only*, or *Off*. The **auto-compaction size cap** (default 8192 MB, 0 = no cap) defers routine full compaction while bounded coalescing is making progress. If clean-up is due and no bounded merge can make progress, *Coalesce, then compact* and *Compact only* automatically run worker-isolated streaming compaction even above that cap instead of showing a clean-up dialog. Choose *Coalesce only* or *Off* when full compaction must never start automatically. The coalescing controls set the largest eligible segment (default 1024 MB), largest batch (default 4096 MB), minimum neighbouring run (default 3), and maximum merges per pass (default 8). Both cleanup paths are bounded streaming operations sized by the build-memory budget, so even a large compaction does not load the whole index into memory. Only accumulated **update history** is coalesced; full-build pages are disjoint data and are skipped. Every folder can override its strategy, size budget, and routine compaction cap under **Manage Indexes ▸ Size…**. A compact-capable index that exceeds its storage budget first attempts streaming compaction; if the compacted base still cannot fit, updates pause and health reports the failure without opening the clean-up dialog. For *Coalesce only* or *Off*, a budget or unreclaimable-history dialog remains available because full compaction is not an automatic option; it offers rebuild, limit/settings changes, one-shot compaction where appropriate, or deletion. Turn on **Stop updating an index that can no longer be cleaned up** if Yagu should pause such a coalesce-only/off index instead of letting it grow. None of these settings can change results: an index that stays segmented or pauses simply prunes less, and anything it cannot safely skip is read live. |
...
**Compaction estimates.** The bottom-right **Index** hover panel and each corresponding card under **Settings ▸ Indexing ▸ Index storage** show an estimated date/time for the next full automatic compaction. For compact-capable modes, an estimate above the routine size cap explains that streaming compaction becomes the automatic fallback if bounded coalescing cannot make progress. A clean-up-attention estimate is reserved for modes that forbid full compaction. Choose **More details** for the observed update-history size, layer count, recent growth rate, threshold, projected index size, automatic-compaction decision, and any smaller coalescing pass expected first. The estimate assumes recent file churn continues and Yagu remains open; coalescing, pausing, closing Yagu, or a burst of file changes can move it. If automatic compaction fails, the existing index remains active, health shows the failure without opening the clean-up dialog, and Yagu waits six hours before attempting the expensive fold again; a successful compaction or rebuild clears that warning.
