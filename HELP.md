...
| Build Scheduling | The build trigger(s), the **update mode**, and separate cadence controls. The trigger is a set of checkboxes — **When enabled**, **At startup**, **When the machine is idle**, **Continuously while Yagu is open**, and **On a schedule** — and you can turn on more than one at once. With none checked the trigger is **Manual**. **Idle delay** appears only when **When the machine is idle** is selected and controls how long Windows must report no keyboard or mouse input before maintenance runs. **Continuous interval** appears only when **Continuously while Yagu is open** is selected and controls the minimum time between maintenance passes; continuous maintenance evaluates immediately when Yagu opens. If both triggers are selected, both controls appear and whichever cadence becomes due first may start the next shared pass. Both paths still honor searches, battery policy, low disk space, and a user pause. Existing settings written before these controls were split keep their prior combined value for both controls. To keep existing indexes continuously current, select **Automatic incremental** (recommended) or **Automatic full rebuild when changed**; the default **Manual full rebuild** update mode only builds missing indexes, regardless of trigger. On a fresh first-launch drive opt-in, Yagu preselects **Continuously while Yagu is open**, a **five-minute continuous interval**, and **Automatic incremental**, while keeping the separate idle delay at its five-minute default and keeping pause-during-search and pause-on-battery enabled; the dialog still lets you override its trigger and update mode. On first load after this update, an existing one-minute continuous interval is migrated once to five minutes; selecting one minute afterward remains supported. The approved first-run profile also uses an 8,000,000-record foreground journal catch-up cap. This tab shows an inline warning if an automatic trigger is ever left on **Manual full rebuild**. **On a schedule** supports an interval (5–10080 minutes) or selected weekdays/time. Automatic incremental applies small append-only deltas and never silently escalates to an expensive full rebuild when journal continuity cannot be proven. The only compatibility exception is a one-time automatic rebuild of an older ReFS index whose stored file identities cannot match ReFS journal records; the old index remains active until the replacement commits, and searches safely live-scan changed content meanwhile. |
| Build Resources | Build memory budget, **build worker parallelism**, serialized folder publication (fixed at one writer), pause-during-search, pause-on-battery, auto-repair, journal catch-up limits, and the incremental-maintenance controls. Build parallelism 0 (default) selects a conservative physical-core-based degree bounded by the memory budget; 1–32 sets an explicit cap. File reads/classification run concurrently, but outcomes are committed in crawl order so content IDs, spill boundaries, and transactional publication remain deterministic. The existing **Limit parallelism on HDDs** safeguard overrides it to one lane for rotational roots. The **post-build catch-up threshold** (default 30,000 journal changes; 0 = any non-empty interval) controls whether a full build immediately applies changes that arrived while its crawl was running. **Recover from a lost change journal by rescanning instead of rebuilding** (on by default) decides what happens when Windows' change journal can no longer prove what changed — its retention window is finite, so an index falls outside it if Yagu is closed for long enough, and a busy system drive can pass that point in only a few hours. Instead of rebuilding the whole folder, Yagu asks each file for its own last-change record and re-reads only the files that actually changed, which typically turns an hours-long rebuild into minutes. This is exact rather than a timestamp guess: the record is written by the file system on every change. A file whose record cannot be read is re-read or read live rather than trusted, and if any folder fails to enumerate the rescan is abandoned in favour of a full rebuild. An elevated session automatically uses a faster whole-volume sweep; Yagu never asks for elevation to do this. A rescan cannot help if the journal was reset or recreated, because that renumbers every record — only a rebuild is safe then. The remaining controls include **use file-system watcher hints** (off by default; a latency hint only — the change journal stays authoritative), the **maximum delta segments** (default 8), and the **compaction size threshold** (default 512 MB). (The opt-in **share aggregate content-index metrics** telemetry toggle lives on the **Privacy** tab.) |
| Index Size Management | How each index reclaims storage. An index only ever grows on its own: every incremental update appends a delta segment, and storage comes back only through **coalescing** (merging a bounded run of neighbouring small segments) or **compaction** (streaming every active layer into a fresh base, which can reclaim more). The **default size-management strategy** picks which an index may use: *Coalesce, then compact* (default), *Coalesce only* (least maintenance work), *Compact only*, or *Off*. The **auto-compaction size cap** (default 8192 MB, 0 = no cap) lets medium-sized indexes compact automatically while leaving hour-scale whole-drive folds user-approved because those consume substantial I/O and temporary disk space. Above the cap only coalescing, an explicitly requested compaction, or a rebuild reclaims space. The four **coalescing** controls set the largest segment eligible to merge (default 1024 MB), the largest merge batch (default 4096 MB — this bounds the work and disk I/O of one merge, not the memory it holds, which is set by the build memory budget; keep it at or above the run minimum multiplied by the segment cap so a full-length run can fit), the fewest neighbouring segments worth merging (default 3), and how many merges one maintenance pass may perform (default 8). Merging and compaction both run as bounded streaming passes: records are ordered on disk in chunks sized by the build memory budget, so folding even a very large index never loads the whole index into memory. Only accumulated **update history** is merged; the pages of a single full build are disjoint parts of one index, so merging them would reclaim nothing and is deliberately skipped. If the largest-segment value is set below your typical segment size, coalescing can never find work and a large index will have no way to reclaim storage. Every folder can override the strategy, the size budget, and the compaction cap under **Manage Indexes ▸ Size…**. If an index does reach its budget and automatic clean-up cannot bring it back under, Yagu says so directly: the index-health row reports **updates paused — reached its N MB size limit**, and a dialog explains in plain language that searches remain complete (uncovered files are read live) but the index is no longer being kept up to date. The dialog fixes it for you — **Rebuild this index** (recommended; frees almost all the space and resumes updates, with the current index still working until the new one is ready), **Raise the limit**, **Compact it instead**, or **Delete this index** — and applies your choice immediately rather than sending you elsewhere. Separately, an index can keep updating and still be unable to shed the history those updates leave behind — when its accumulated update history passes the clean-up thresholds and neither merging nor compacting is permitted at its size. That row reports **still updating, but N MB of accumulated update history cannot be reclaimed automatically**, and its dialog offers **Compact now** (recommended; a one-shot, user-approved streaming fold that does not change your automatic-compaction cap), **Rebuild this index**, **Delete this index**, or **Review size settings**. Turn on **Stop updating an index that can no longer be cleaned up** if you would rather Yagu paused that index than let it grow — it is off by default, because pausing costs search speed. None of these settings can change search results — an index that stays segmented, or one paused at its budget, simply prunes less, and anything it cannot prove safe to skip is read live. |
...
**Compaction estimates.** The bottom-right **Index** hover panel and each corresponding card under **Settings ▸ Indexing ▸ Index storage** show an estimated date/time for the next full automatic compaction. If the projected index will be above its automatic-compaction cap, the same line estimates when clean-up attention is likely instead. Choose **More details** for the observed update-history size, layer count, recent growth rate, threshold, projected index size, automatic-compaction decision, and any smaller coalescing pass expected first. The estimate assumes recent file churn continues and Yagu remains open; coalescing, pausing, closing Yagu, or a burst of file changes can move it.

**Index size history graph.** The graph button at the top-right of the bottom status bar's **Indexing overview** opens a line graph of physical index storage over time. **All disks** shows the collective size of every stored index; the filter can instead show the indexes whose source folders belong to one observed drive. Yagu records at most one snapshot per hour during its existing off-thread all-drive health refresh, so opening the graph never starts another index scan. **Settings ▸ Indexing ▸ Storage ▸ Index size history retention** controls the visible and retained window from 1–365 days (default 30). The bounded history is stored locally as `index-size-history.json` beside the index scopes and is never telemetry.
...
The CLI exposes the same lifecycle through `--build-index`, `--rebuild-index`, `--index-status`, `--compact-index`, `--validate-index`, `--delete-index`, `--clear-indexes`, root/filter commands, and `--index-config`; see **Content Index (CLI)** below.
...
| Max rendered matches per section | Ceiling on matches or lines a single section renders across all "Load more" expansions (default: 40,000). When reached, the preview offers to raise the saved limit by 40,000 and continue in bounded chunks, or open the file in the editor. |
...
Both the directory box and the search pattern box have small controls tucked inside their trailing edge — the pin/index/Browse buttons on the directory box, and the Match Case / Regex / Multiline / Exact toggles on the search pattern box. Tab therefore has two sensible destinations.

The first time you press **Tab** in either box, a small callout appears asking which you want:

- **Go to the control inside the box** — the pin button from the directory box, Match Case from the search pattern box.
- **Skip past it** — jump straight to the search pattern box (from the directory box) or to the **Search** button (from the search pattern box).

Your answer is remembered and applied to every later Tab press from that box. The callout always names whichever control currently sits first inside the box, so it stays accurate if the button row changes. To be asked again, use **Settings ▸ Reminders and Warnings ▸ Reset Tab destination prompts**.

**Shift+Tab** is unaffected and always moves backwards through the normal tab order.
...
| `--index-status [<path>]` | Print the index manifest, stored content-record/layer counts, creation time, active-generation build time, true last incremental-update time (when one exists), and last-build skip/fail summary for a scope. It also breaks the active layers into **base**, **full-build pages**, and **update history**, and says whether a clean-up is due and whether it can still happen automatically. |
| `--compact-index [<path>]` | Fold every active layer of one index into a fresh single index. Runs as a bounded streaming pass in the maintenance worker — records are ordered on disk in chunks sized by the build memory budget, so it never loads the index into memory. It ignores the automatic-compaction size cap for this run only and never changes that setting. Cancelling leaves the existing index untouched. |
| `--validate-index [<path>]` | Structurally validate a stored index — checksum every active layer, walk its records, and confirm the counts agree — then print the stored record and segment counts. Read-only: it never modifies the index. When validation fails it prints why and reminds you those searches live-scan. The CLI counterpart of **Settings ▸ Indexing ▸ Validate**. |
...
| `--index-set-root-size <path> [--root-size-mode <mode>] [--root-size-budget-mb <n>] [--root-auto-compaction-cap-mb <n>]` | Set **per-folder** size management for one index. `<path>` must already be a registered indexed folder (see `--index-list-roots`); anything else is rejected rather than saved where you could never see it again. `<mode>` is `Off`, `Coalesce`, `Compact`, or `CoalesceThenCompact` — an unrecognized mode or a non-numeric size is reported and ignored instead of quietly applying a different value. Use `-1` for either numeric value to inherit the global setting, `0` for "no limit"/"no cap". At its size budget Yagu reclaims what it safely can and then pauses that index's updates instead of letting it grow; searches still return every match, because files the index no longer covers are read live. |
| `--index-clear-root-size <path>` | Remove one index's size override so it follows the global settings again. Says so plainly when there was no override to remove. |
