...
| `--index-config <key>=<value>` | Set an Indexing setting (repeatable; validated exactly like the Settings tab — unknown keys or invalid values fail without saving any change). Build-output changes print the affected roots and exact `--rebuild-index` commands. Example: `--index-config IndexPostBuildCatchUpThresholdChanges=30000`. |
