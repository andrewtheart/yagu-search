            ...
            if (!_fileIndex.HasValue)
            {
                ...
                if (_actualIndexes == null || _actualIndexes.Contains(index))
                {
                    ...
                    try
                    {
                        ...
                        // Zip Slip protection: canonicalize the path and verify it stays within the extraction directory
                        var canonicalPath = Path.GetFullPath(fileName);
                        var canonicalDir = Path.GetFullPath(_directory!);
                        if (!canonicalDir.EndsWith(Path.DirectorySeparatorChar.ToString()))
                        {
                            canonicalDir += Path.DirectorySeparatorChar;
                        }
                        if (!canonicalPath.StartsWith(canonicalDir, StringComparison.OrdinalIgnoreCase))
                        {
                            throw new SharpSevenZipArchiveException("Entry path escapes the extraction directory (potential Zip Slip attack).");
                        }
                    ...
                    if (!NativeMethods.SafeCast(data, false))
                    {
                        ...
                        if (File.Exists(fileName))
                        {
                            ...
                            {
                                // Validate replacement path against Zip Slip as well
                                var replacementPath = Path.GetFullPath(fnea.FileName);
                                var safeDir = Path.GetFullPath(_directory!);
                                if (!safeDir.EndsWith(Path.DirectorySeparatorChar.ToString()))
                                {
                                    safeDir += Path.DirectorySeparatorChar;
                                }
                                if (!replacementPath.StartsWith(safeDir, StringComparison.OrdinalIgnoreCase))
                                {
                                    AddException(new SharpSevenZipArchiveException("Replacement path escapes the extraction directory."));
                                    outStream = _fakeStream;
                                    return 0;
                                }
