...
internal sealed partial class CoreTextControlBox : UserControl
{
    ...
    public CoreTextControlBox()
    {
        ...
        // Option B diagonal (two-axis) precision-touchpad panning: wire the composition InteractionTracker
        // once the control (and its selection canvas' visual) is in the tree. See CoreTextControlBox.DiagonalScroll.cs.
            ...
            case VirtualKey.Down:
                {
                    if (shift)
                    {
                        selectionManager.StartSelectionIfNeeded();
                        if (WordWrap)
                            textRenderer.MoveCursorByVisualRows(Canvas_Text, cursorManager.currentCursorPosition, 1);
                        else
    ...
    public new FontFamily FontFamily { get => textManager._FontFamily; set { textManager._FontFamily = value; textRenderer.ClearWrapCache(); textRenderer.NeedsTextFormatUpdate = true; canvasUpdateManager.UpdateAll(); } }
    ...
    public bool WordWrap
    {
        ...
        set
        {
            ...
            textRenderer.NeedsUpdateTextLayout = true;
            lineNumberRenderer.NeedsUpdateLineNumbers();
            zoomManager.ZoomNeedsRecalculateLongestLine = true;
            if (value)
    ...
    {
        get => new ScrollBarPosition(HorizontalScroll, VerticalScroll);
        set { HorizontalScroll = value.ValueX; VerticalScroll = value.ValueY; }
    }
    ...
    public double VerticalScroll { get => scrollManager.VerticalScroll; set => scrollManager.VerticalScroll = value; }
    public double HorizontalScroll { get => WordWrap ? 0 : scrollManager.HorizontalScroll; set => scrollManager.HorizontalScroll = WordWrap ? 0 : value; }
