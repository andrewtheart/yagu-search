...
internal class PointerActionsManager
{
    ...
    public void PointerWheelAction(ZoomManager zoomManager, PointerRoutedEventArgs e)
    {
        ...
        //Zoom using mousewheel or pinch-to-zoom. A precision-touchpad pinch is
        //delivered by Windows as a Ctrl-modified wheel, but that Control comes
        //from the wheel message (e.KeyModifiers), not the physical keyboard
        ...
        else if (Utils.IsKeyPressed(VirtualKey.Shift))
        {
            ...
            {
                scrollManager.HorizontalScroll -= delta * scrollManager._HorizontalScrollSensitivity;
                needsUpdate = true;
            }
        }
        ...
        else
        {
            ...
            int currentScrollRow = (int)(scrollManager.verticalScrollBar.Value / Math.Max(1, textRenderer.SingleLineHeight) * scrollManager.DefaultVerticalScrollSensitivity);
