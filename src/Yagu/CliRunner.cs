...
internal static class CliRunner
{
    ...
    private static void PrintSemanticExplanation(CliArgs args, ResolvedSearchPlan resolved)
    {
        ...
        if (resolved.ModifiedAfterDate is { } ma) o.WriteLine($"  modified-after : {ma:yyyy-MM-dd}");
        if (resolved.ModifiedBeforeDate is { } mb)o.WriteLine($"  modified-before: {mb:yyyy-MM-dd}");
        if (resolved.MaxSearchDepth is { } depth) o.WriteLine($"  max-depth      : {(depth <= 0 ? "unlimited" : depth.ToString(CultureInfo.InvariantCulture))}");
        if (resolved.ObeyGitignore is { } gi)     o.WriteLine($"  obey-gitignore : {gi}");
    ...
    private static SearchOptions BuildSearchOptions(CliArgs args, AppSettings s, string? directoryOverride = null, int? parallelismOverride = null, FileListerBackend? backendOverride = null)
    {
        ...
        return new SearchOptions
        {
            ...
            MultilineEngine       = (MultilineEngineKind)(args.MultilineEngineIndex ?? 0),
    ...
    private static async Task<int> RunSearchAsync(List<SearchOptions> perRootOptions, CliArgs args, bool vtEnabled)
    {
        ...
        // Multiline runs through the per-file ContentSearcher path (the native streaming scanner is
        // not used for multiline), so the native DirectOutputSink cannot emit plain CLI output. When
        // not collecting for post-processing, route each streamed SearchEvent.Match/MatchBatch through
        // a managed GrepStyleWriter as it arrives (§4 hidden gate) — whether the match came from the
        // native multiline engine or the managed fallback.
        ...
        Console.Out.WriteLine("""
            ...
            MATCHING:
              ...
              -U, --multiline             Match across lines: run the pattern over the whole file so a
                  ...
                  --multiline-engine <e>  Native multiline backend: regex (default) | grep. Both give
                                          identical results — a performance knob (implies --multiline).
...
internal sealed class CliArgs
{
    ...
    public int?             MultilineEngineIndex { get; private set; }
    ...
    public static CliArgs Parse(string[] raw)
    {
        ...
        while (i < raw.Length)
        {
            ...
            if (TryGetVal(raw, ref i, out v, "--multiline-engine"))
            {
                // Selecting an engine implies multiline (a pure performance knob under -U).
                a.Multiline = true;
                a.MultilineEngineIndex = v.ToLowerInvariant() switch
                {
                    "grep" or "grep-searcher" or "ripgrep" => 1,
                    _ => 0, // "regex" (default hand-rolled regex::bytes)
                };
                continue;
            }
